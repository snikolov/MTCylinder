...
DP rtsafe(void funcd(const DP, DP &, DP &), const DP x1, const DP x2,
 ...
 void rzextr(const int iest, const DP xest, Vec_I_DP &yest, Vec_O_DP &yz,
	     Vec_O_DP &dy);
 DP sample_gauss(double sigma);
 double* sample_gauss2(double sigma, double* empty_sample);
 double* sample_circle(double* empty_sample);
 double* sample_metropolis(double (*fp)(double,double), double* sample);
